...
.EXAMPLE
.\Get-MailboxReport.ps1 -All -SendEmail -MailFrom exchangereports@epiqsystems.com -MailTo kbennett@epiqsystems.com -MailServer mailrelay.amer.epiqcorp.com
...
foreach ($mb in $mailboxes)
{
   ...
   # $deleteditemsstats = Get-MailboxFolderStatistics $mb -FolderScope DeletedItems | Where {$_.FolderPath -eq "/Deleted Items"}
	...
	$user = Get-User $mb -IgnoreDefaultScope
$PSEnv3 = PowerShell {	
	$aduser = Get-ADUser $mb.samaccountname -Properties Enabled,AccountExpirationDate,whenCreated,whenChanged,LastLogonTimeStamp -server ($mb.Identity.ToString() | %{$_.Split("/")[0]})
    }
	...
	$userObj | Add-Member NoteProperty -Name "whenCreated" -Value $aduser.whenCreated
	$userObj | Add-Member NoteProperty -Name "AccountExpirationDate" -Value $aduser.AccountExpirationDate
	$userObj | Add-Member NoteProperty -Name "whenChanged" -Value $aduser.whenChanged
	$userObj | Add-Member NoteProperty -Name "LastLogonTimeStamp" -Value ([DateTime]::FromFileTime($aduser.LastLogonTimeStamp))
	$userObj | Add-Member NoteProperty -Name "DistinguishedName" -Value $mb.DistinguishedName
	$userObj | Add-Member NoteProperty -Name "LitigationHoldEnabled" -Value $mb.LitigationHoldEnabled
