...
Function Check-Enabled {
    ForEach ($user in $data){
        Get-Aduser $user.UserName -Server apac.epiqcorp.com | Select SamAccountName,Enabled | Export-csv c:\temp\run.csv -NoTypeInformation -Append -Force
    }
}
...
Function MigrateUser {
    ForEach ($user in $data) {
        ...
        $account = $dn.SamAccountName + "@epiqsystems.com" #Change address to the .co.uk or .hk
