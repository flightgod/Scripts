...
function updateState {
    foreach ($Script:name in $import){
        #$UserStateFind = get-AdUser $name.Username -Properties * | Select Name, State
        
            #Set-ADUser $name.Username -State "England" -Credential $UserCredential -Server $DomainController
            get-ADUser $name.Username -Properties * -Server $DomainController | Select Name, State
    }
