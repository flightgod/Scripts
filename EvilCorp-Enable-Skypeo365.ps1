...
$UKDomainController = "P054ADSEUDC01.EURO.EvilCorpCORP.COM"
$HKDomainController = "P054ADSAPDC01.APAC.EvilCorpCORP.COM"
...
Function Get-User {
    ...
    Set-ADUser -Identity $User -Add @{'msRTCSIP-PrimaryHomeServer' = "CN=Lc Services,CN=Microsoft,CN=1:1,CN=Pools,CN=RTC Service,CN=Services,CN=Configuration,DC=EvilCorpCORP,DC=COM"} -Server $DomainController
    Set-ADUser -Identity $User -Add @{'msRTCSIP-PrimaryUserAddress' = "sip:$User@EvilCorpsystems.com"} -Server $DC
    Set-ADUser -Identity $User -Add @{'msRTCSIP-UserEnabled' = "TRUE"} -Server $DC

    $Global:email = $user.Mail
    $Global:sip = "$user@EvilCorpsystems.com"
...
Function Get-User-UK {
    ...
    $Script:upn = $account+"@EvilCorpsystems.co.uk"
...
Function Get-User-HK {
    ...
    $Script:upn = $account+"@EvilCorpsystems.com.hk"
    $Script:email = $account+"@EvilCorpsystems3.mail.onmicrosoft.com"
    $Script:sip = "SIP:"+$upn
    $Script:DC = $HKDomainController
...
do
 {
     ...
     switch ($selection)
     {
         '1' {
             ...
             .\EvilCorp-Check-SkypeSettings.ps1
         } '3' {
             Show-Menu
         } '4' {
            Migrate-User
...
{
    param (
        [string]$Title = 'EvilCorp o365 Skype Script'
    )
    Clear-Host
    Write-Host "================ $Title ================"
