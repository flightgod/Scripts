...
.FUNCTIONALITY
    ...
    1. Check if there are disabled users in the EvilCorp-All, and EvilCorp-All-US DL
    ...
    7. Removes users from the EvilCorp-All and EvilCorp-All-US Groups
...
# Variables
param (
$Global:SingleGroup = "EvilCorp-All",
$Global:USGroup = "EvilCorp-All-US",
$OU = "OU=Standard,OU=Employees,OU=Corp IT,DC=amer,DC=EvilCorpcorp,DC=com",
$OUFull = "DC=amer,DC=EvilCorpcorp,DC=com",
...
Function GetInfo {
$Global:NewGroup = Get-ADGroup "EvilCorp-All" -Properties Member -Server $DomainController | 
...
foreach($user in $NewGroup) {
    try {
        ...
        try {
            $s2 = get-ADUser $user -Server euro.EvilCorpcorp.com
            If($s2.Enabled -eq $false) {
                ...
                #Remove-ADGroupMember -Identity $SingleGroup -Members $S1.SamAccountName -Server euro.EvilCorpcorp.com -Confirm:$False
                write-host $array.count
            }
        } catch {
            write-host $user " - Not found in Euro, checking APAC" -ForegroundColor DarkYellow
            try {
                $s3 = get-ADUser $user -Server apac.EvilCorpcorp.com
                If($s3.Enabled -eq $false) {
                    Write-Host $user.SamAccountName " - Disabled, should remove from" $SingleGroup -ForegroundColor Red
                    $Global:array += $s3.SamAccountName
                    #Remove-ADGroupMember -Identity $SingleGroup -Members $S1.SamAccountName -Server apac.EvilCorpcorp.com -Confirm:$False
                    write-host $array.count
                } 
             } catch {
...
# Gets users that are disabled that are members of EvilCorp-ALL-US
Function GetUSInfo {
...
Function CheckMBExists {
    ...
    $Global:GroupMembers = Get-ADGroup "EvilCorp-All" -Properties Member -Server $DomainController | Select-Object -ExpandProperty Member
...
Function RemoveFromEvilCorpAll {
...
If ($USarray.count -gt 0){
    Write-Host "Going to delete" $USarray.count "users from Group EvilCorp-All-US"
        $Global:UScount = $USarray.count
        foreach ($USBadUser in $USarray){
            $UScount
            Write-Host "Removing user " $USBadUser "from EvilCorp-ALL-US" -ForeGroundColor Green
            Remove-ADGroupMember -Identity "EvilCorp-All-US" -Members $USBadUser -Server $DomainController -Confirm:$False
            $UScount=$UScount-1
        } 
    } Else {
        Write-Host "No users to delete - wooho"
...
Function CheckforDisabledAccounts {
    $Global:GetList = Get-ADUser -Filter * -SearchBase $OU -Properties EmployeeID -Server amer.EvilCorpcorp.com | `
...
Function CheckforALLDisabledAccounts {
    $Global:FullList = Get-ADUser -Filter * -SearchBase $OUFull -Properties EmployeeID -Server amer.EvilCorpcorp.com | `
    Where-Object {$_.Enabled -eq $false} | `
    Select-Object SAMAccountName, EmployeeID
...
# Body text Function
Function BodyText {
    $USUsers = "Users in EvilCorp-All:" + $NewGroup.count
    $Variable1 = "Number of users to remove from" + $SingleGroup + ": " + $array.Count
    $Variable2 = "Users in EvilCorp-All-US Currently:" + $USCount.count
...
Write-host "Checking EvilCorp-All-US....."

GetUSInfo # Function to get info for EvilCorp-All-US

Write-Host "Script has taken" $stopwatch.Elapsed.Hours "hour(s)," $stopwatch.Elapsed.Minutes "Minutes and"$stopwatch.Elapsed.Seconds "Seconds to Run so far"
  
$USCount = Get-ADGroupMember "EvilCorp-All-US" -Server $DomainController

write-host "Users in EvilCorp-All:" $NewGroup.count
Write-Host "Number of users to remove from" $SingleGroup ": " $array.Count
$array

Write-host "Users in EvilCorp-All-US Currently:" $USCount.count
write-Host "Number of Users to remove from EvilCorp-All-US: " $USArray.count
$USArray

RemoveFromEvilCorpAll # Will remove users from EvilCorp-All
RemoveFromUS # Will remove users from EvilCorp-ALL-US
...
CheckMBExists # Checks that we can find mailboxes for all users in EvilCorp-All
$NoMBResults
BodyText
SendEmail
